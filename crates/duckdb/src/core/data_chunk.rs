use super::{
    ...
    vector::{
        ArrayVector, BorrowGuard, FlatVector, ListVector, ResultExt, StructVector, VectorRef, VectorState,
...
impl DataChunkHandle {
    /// Wrap a DuckDB-produced callback input chunk without taking ownership.
    ///
    /// # Safety
    ///
    /// `ptr` must be a valid input `duckdb_data_chunk` that stays allocated and
    /// unmutated by other code for the full lifetime of the returned handle and
    /// any vectors derived from it. Its current cardinality must describe the
    /// initialized span of every top-level vector.
    #[cfg(feature = "vtab")]
    #[cfg_attr(not(feature = "vscalar"), allow(dead_code))]
    pub(crate) unsafe fn new_unowned_input(ptr: duckdb_data_chunk) -> Self {
        let capacity = unsafe { duckdb_data_chunk_get_size(ptr) as usize };
        Self {
            ptr,
            owned: false,
            capacity,
            state: VectorStateCell::new(VectorState::CallbackInput { readable_len: capacity }),
        }
    }

    /// Wrap a callback output chunk with DuckDB's standard backing capacity.
    ///
    /// # Safety
    ///
    /// `ptr` must be the output chunk supplied by DuckDB to a table-function
    /// callback and remain live for the returned handle's use.
    #[cfg(feature = "vtab")]
    pub(crate) unsafe fn new_unowned_output(ptr: duckdb_data_chunk) -> Self {
        Self {
            ptr,
            owned: false,
    ...
    /// the column already has an active compatibility view.
    pub fn list_vector(&self, idx: usize) -> ListVector<'_> {
        ListVector::from_vector(self.compatibility_vector(idx)).or_panic()
    }
    ...
    pub fn array_vector(&self, idx: usize) -> ArrayVector<'_> {
        ArrayVector::from_vector(self.compatibility_vector(idx)).or_panic()
    }
    ...
    /// # Panics
    ///
    /// Panics if `idx` is out of range, the column is not `STRUCT`, or the
    /// column already has an active compatibility view.
    pub fn struct_vector(&self, idx: usize) -> StructVector<'_> {
        StructVector::from_vector(self.compatibility_vector(idx)).or_panic()
    }

    /// Builds a compatibility view for the legacy family-specific interface.
    ///
    /// The pointer-keyed lease prevents two safe writable-capable views of the
    /// same vector. Views of callback input are readable; newly allocated and
    /// callback output chunks remain under construction. The legacy Arrow
    /// reader joins this lease, while native writable access requires an
    /// exclusive borrow of the chunk.
    fn compatibility_vector(&self, idx: usize) -> VectorRef<'_> {
        ...
        // The pointer-keyed guard closes the compatibility interface's
    ...
    pub unsafe fn assume_initialized(&mut self) {
        unsafe { self.mark_initialized() };
    }

    pub(crate) unsafe fn mark_initialized(&mut self) {
        self.state.set(VectorState::Initialized {
...
mod test {
    ...
    fn test_duplicate_column_vector_view_is_rejected() {
        ...
        .unwrap_err();
        assert!(panic_payload(duplicate.as_ref()).contains("already has an active compatibility view"));
    ...
    #[test]
    fn test_raw_vector_alias_uses_the_same_lease() {
        let datachunk = DataChunkHandle::new(&[LogicalTypeHandle::from(LogicalTypeId::Bigint)]);
        let mut ptr = unsafe { duckdb_data_chunk_get_vector(datachunk.ptr, 0) };
        let mut adapter = unsafe { crate::core::WritableVectorRef::from_raw(&mut ptr, datachunk.capacity) }.unwrap();
        let raw = crate::core::WritableVector::flat_vector(&mut adapter);
        ...
        drop(raw);
        drop(adapter);
        let _chunk_view = datachunk.flat_vector(0);
    }
    ...
    #[test]
    #[cfg(feature = "vtab")]
    fn test_callback_input_capacity_is_its_cardinality() {
        ...
        assert_eq!(input.flat_vector(0).capacity(), 3);
        assert!(input.initialized_vector(0, 4).is_err());
    }

    #[test]
    #[cfg(feature = "vtab")]
    fn test_callback_input_rejects_safe_mutation() {
        let mut owner = DataChunkHandle::new(&[LogicalTypeHandle::from(LogicalTypeId::Varchar)]);
        ...
        assert!(input.try_set_len(0).is_err());
        let mutation = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            input.flat_vector(0).insert(0, "mutated");
        }))
        .unwrap_err();
